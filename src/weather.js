...
async function fetchWeather() {
    // Get location
    const geoLoc = document.querySelector('#search').value;
    ...
    try {
        // Fetch request to REST API
        const response = await fetch(`./../API/thunderFetcher.php?location=${geoLoc}`);
...
// Search weather on click/enter btn
searchBtn.addEventListener('click', fetchWeather);
document.addEventListener('keypress', e => e.key === 'Enter' && fetchWeather());
